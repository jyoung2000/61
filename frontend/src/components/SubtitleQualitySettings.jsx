...
export default function SubtitleQualitySettings() {
  ...
  const save = async () => {
    ...
    try {
      ...
      delete body.openai_configured;
      delete body.groq_configured;
      ...
      setForm((f) => ({
        ...f, ...d,
        google_translate_api_key: '', deepl_api_key: '', openai_api_key: '',  // never echo back
      }));
      setSaved(true);
    } catch {
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 8 }}>
          <div style={{ flex: '1 1 260px' }}>
            <label style={labelStyle}>Where transcription runs</label>
            <select
              style={selectStyle} value={form.transcription_provider || 'local'}
              onChange={(e) => set('transcription_provider', e.target.value)}
            >
              <option value="local">Local (faster-whisper on this machine — default)</option>
              <option value="groq">Groq (whisper-large-v3-turbo — very fast{form.groq_configured ? '' : ', needs GROQ_API_KEY'})</option>
              <option value="openai">OpenAI (whisper-1 / gpt-4o-transcribe{form.openai_configured ? '' : ', needs API key'})</option>
            </select>
          </div>
          {form.transcription_provider === 'openai' && !form.openai_configured && (
            <div style={{ flex: '1 1 260px' }}>
              <label style={labelStyle}>OpenAI API key</label>
              <input
                style={inputStyle} type="password" placeholder="sk-..."
        ...
        <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>
          Cloud output flows through the same hallucination filter, forced
          alignment, polish and formatter as local — quality rules are
          uniform. Falls back to local automatically on API failure.
        </div>
      </div>
