...
def translation_plan(model_name, n_cues, *, is_ollama=True, companion_parallel=1) -> dict:
    ...
    # Cloud providers have no VRAM/KV-slot constraint — the Companion-derived
    # parallelism (1 with no Companion at all) starves them. Run a modest
    # client-side fan-out; the provider's own rate limiter still spaces the
    # actual submissions, so this never turns into a request flood.
    cloud_cc = max(1, int(getattr(settings, "TRANSLATION_CLOUD_CONCURRENCY", 3) or 3))
    ...
    if not is_large:
        return {"batch": min(small_batch, n), "num_ctx": small_ctx,
                "concurrency": cp if is_ollama else max(cp, cloud_cc)}
    return {
        ...
        "concurrency": (
            min(cp, max(1, int(getattr(settings, "TRANSLATION_LARGE_CONCURRENCY", 1) or 1)))
            if is_ollama else max(cp, cloud_cc)),
    }
