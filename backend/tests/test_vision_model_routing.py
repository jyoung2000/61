...
Exercises ``select_primary_model_for_content`` (the routing helper)
and ``OpenRouterProvider.apply_primary_model_override`` (which mutates
...
from backend.services.providers.openrouter_provider import (
    PRESETS,
    select_primary_model_for_content,
)
...
def test_routing_anime_picks_qwen3vl_in_premium():
    model = select_primary_model_for_content("anime", PRESETS["premium"]["vision"])
    assert model == QWEN3_VL


def test_routing_animation_picks_qwen3vl_in_balanced():
    model = select_primary_model_for_content("animation", PRESETS["balanced"]["vision"])
    assert model == QWEN3_VL


def test_routing_cartoon_picks_qwen3vl():
    model = select_primary_model_for_content("cartoon", PRESETS["premium"]["vision"])
    assert model == QWEN3_VL


def test_routing_gameplay_picks_qwen3vl_in_balanced():
    model = select_primary_model_for_content("gameplay", PRESETS["balanced"]["vision"])
    assert model == QWEN3_VL


def test_routing_gameplay_fps_picks_qwen3vl():
    model = select_primary_model_for_content("gameplay_fps", PRESETS["premium"]["vision"])
    assert model == QWEN3_VL


def test_routing_talking_head_uses_preset_default():
    model = select_primary_model_for_content("talking_head", PRESETS["premium"]["vision"])
...
def test_routing_music_video_uses_preset_default():
    ...
    model = select_primary_model_for_content("music_video", PRESETS["premium"]["vision"])
    assert model == PRESETS["premium"]["vision"]


def test_routing_narrative_uses_preset_default():
    model = select_primary_model_for_content("cinematic_dialogue", PRESETS["balanced"]["vision"])
    assert model == PRESETS["balanced"]["vision"]


def test_routing_none_uses_preset_default():
    """No content type → preset default; never guess."""
    model = select_primary_model_for_content(None, PRESETS["premium"]["vision"])
    assert model == PRESETS["premium"]["vision"]


def test_routing_unknown_content_type_uses_preset_default():
    model = select_primary_model_for_content("random_nonsense", PRESETS["balanced"]["vision"])
    assert model == PRESETS["balanced"]["vision"]
...
def test_routing_enum_like_object():
    ...
    class FakeEnum:
        value = "anime"
    model = select_primary_model_for_content(FakeEnum(), PRESETS["premium"]["vision"])
    assert model == QWEN3_VL
...
def test_apply_primary_model_override_swaps_and_restores():
    """The provider-level override mutates _primary_model and adds the
    ...
    with patch.object(
        ...
        mock_settings.OPENROUTER_PRIMARY_MODEL = ""
        mock_settings.OPENROUTER_EDITORIAL_MODEL = ""
        ...
        assert provider._primary_model == PRESETS["premium"]["vision"]
        base_before = provider._base_primary_model
        ...
        returned = provider.apply_primary_model_override("anime")
        assert returned == QWEN3_VL
        assert provider._primary_model == QWEN3_VL
        assert base_before in provider._vision_fallbacks

        # Calling again with an override-target type is idempotent.
        provider.apply_primary_model_override("anime")
        assert provider._primary_model == QWEN3_VL
        # No-op for a non-override content type — returns the current
        # _primary_model (which is Qwen3-VL because we routed earlier),
        # but the override mechanism sets _base_primary_model as the
        # fallback so safety is preserved via the fallbacks list.
        returned_th = provider.apply_primary_model_override("talking_head")
...
def test_apply_primary_model_override_logs_routing_decision(caplog):
    ...
    with patch.object(
        ...
        with caplog.at_level(
            ...
            provider.apply_primary_model_override("gameplay")
