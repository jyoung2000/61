...
# ── Vision-offload install proxy + public model route ──────────────────────

def test_vision_model_route_is_public_but_control_routes_are_not():
    """The Companion (a cookieless LAN peer) streams the YOLO-World weight
    during its from-source install, so the model route MUST bypass auth like
    the installer artifacts — otherwise the download 401s (the observed
    failure). The install/status CONTROL routes stay behind auth."""
    from backend.app.auth import middleware as mw
    assert mw._is_public_path("/api/downloads/companion/vision-model") is True
    assert mw._is_public_path("/api/downloads/companion/vision-install") is False
    assert mw._is_public_path("/api/downloads/companion/vision-install/status") is False


def test_vision_install_no_companion(monkeypatch):
    from backend.services import ollama_registry as reg
    monkeypatch.setattr(reg, "companion_host", lambda: None)
    with pytest.raises(Exception):  # HTTPException 400 — no paired Companion
        asyncio.run(D.companion_vision_install())


def test_vision_install_started(paired_reg, monkeypatch):
    _fake_httpx(monkeypatch, {"/v1/vision/install": _FakeResp(202, "started")})
    out = asyncio.run(D.companion_vision_install())
    assert out["started"] is True


def test_vision_install_status_passthrough(paired_reg, monkeypatch):
    _fake_httpx(monkeypatch, {
        "/v1/vision/install/status": _FakeResp(200, {
            "active": True, "stage": "torch", "percent": 20.0,
            "message": "Installing PyTorch…", "healthy": False})})
    out = asyncio.run(D.companion_vision_install_status())
