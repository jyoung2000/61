...
def test_select_ollama_vision_returns_default_for_none():
    ...
    assert select_ollama_primary_model_for_content(None, "moondream:1.8b") == "moondream:1.8b"
...
def test_select_ollama_vision_returns_default_for_unknown():
    ...
    assert select_ollama_primary_model_for_content("podcast", "moondream:1.8b") == "moondream:1.8b"
...
def test_select_ollama_vision_routes_known_types(content_type, expected_substr):
    ...
    result = select_ollama_primary_model_for_content(content_type, "moondream:1.8b")
...
def test_select_ollama_vision_case_insensitive():
    ...
    assert "minicpm" in select_ollama_primary_model_for_content(
        "ANIME", "moondream:1.8b",
    ).lower()


def test_apply_primary_model_override_idempotent():
    ...
    p = OllamaProvider()
    initial = p._primary_model
    r1 = p.apply_primary_model_override("anime")
    r2 = p.apply_primary_model_override("anime")
    assert r1 == r2
    # And reverting to no-override goes back to the base.
    p.apply_primary_model_override(None)
    assert p._primary_model == initial


def test_apply_primary_model_override_swaps_for_anime():
    from backend.services.providers.ollama_provider import OllamaProvider

    p = OllamaProvider()
    initial = p._primary_model
    p.apply_primary_model_override("anime")
    assert p._primary_model != initial
    assert "minicpm" in p._primary_model.lower()
